...
const Footer = ({isDisable,setIsDisable}) => {
  ...
  const SaveDraft = ()=>{
    const error = data.map((items)=>items?.isQuestion === false);
    if(error) {
  ...
  return (
    ...
    fixed bg-[#F6F8FAE5]/90  bottom-0 lg:w-[640px] border-t">
      <button onClick={SaveDraft} disabled={isDisable} className={` ${isDisable ? 'cursor-not-allowed' : 'cursor-pointer custom-shadow-draft '}  flex items-center justify-center pt-[6px] pr-[16px] pb-[6px] pl-[14px] gap-[4px] border-1 rounded-[12px] bg-[#fff] `}>
       {
